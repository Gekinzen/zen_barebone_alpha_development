#!/usr/bin/env python3
# ~/.config/hypr-control-center/widgets/system_monitor.py
"""
...
class GraphWidget(Gtk.DrawingArea):
    ...
    def _draw(self, area, cr, width, height):
        ...
        # Draw graph line
        cr.set_source_rgba(0.04, 0.52, 1.0, 0.8)  # Blue
...
class SystemMonitorWidget(BaseWidget):
    def __init__(self):
        ...
        print(f"[sysmon] 🎮 GPU: {self.gpu_name}")
        ...
        # Update stats every 1 second for smooth graphs
        self.update_stats()
    ...
    def detect_gpu(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return gpu.replace('NVIDIA GeForce ', '').replace('NVIDIA ', '')[:18]
        except:
            pass
                ...
                for line in result.stdout.split('\n'):
                    if 'VGA' in line or 'Display' in line:
                        if 'AMD' in line or 'Radeon' in line:
                            match = re.search(r'(RX \d+\s?\w*|Radeon.*?)(\[|$)', line)
                            if match:
                                return match.group(1).strip()[:18]
                        elif 'Intel' in line:
                            match = re.search(r'(HD|UHD|Iris).*?(\d+)?', line)
                            if match:
                                return f"Intel {match.group(0)}"[:18]
        except:
            pass
    ...
    def update_stats(self):
        ...
        try:
            ...
            # GPU
            gpu_temp, gpu_usage_str = self.get_gpu_stats()
            try:
                gpu_percent = float(gpu_usage_str.replace('%', '')) if '%' in gpu_usage_str else 0
            except:
                gpu_percent = 0
            self.gpu_card['graph'].add_data_point(gpu_percent)
            self.gpu_card['left_value'].set_text(gpu_temp)
            self.gpu_card['right_value'].set_text(gpu_usage_str)
    ...
    def get_network_speed(self):
        ...
        try:
            ...
            if self.prev_net_io:
                ...
                down_bps = down_bytes / 1
                up_bps = up_bytes / 1
                
                down_str = self.format_speed(down_bps)
                up_str = self.format_speed(up_bps)
                
                # Calculate percentage for graph (scale: 10MB/s = 100%)
                if 'MB/s' in down_str:
