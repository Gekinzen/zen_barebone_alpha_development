...
class ThemeManager:
        ...
        if self.theme_file.exists():
            try:
                with open(self.theme_file, 'r') as f:
                    data = json.load(f)
                    return data.get("theme_source", "custom")
            except:
                pass
        return "custom"
    
    def set_theme_source_mode(self, mode: str):
        """Set theme source mode"""
        data = {}
        if self.theme_file.exists():
            try:
                with open(self.theme_file, 'r') as f:
    ...
    def _apply_to_waybar(self, theme_id: str):
        """Apply theme to Waybar with smart styling"""
        colors = self.THEMES[theme_id]["colors"]
        ...
        # Generate complete waybar theme CSS
        waybar_css = self._generate_waybar_theme_css(theme_name, colors)
    ...
    def _generate_waybar_theme_css(self, theme_name: str, colors: dict) -> str:
        """Generate complete Waybar CSS with smart color assignments"""
        
        # Determine hover color (usually a bright accent)
        hover_color = colors.get("red", colors.get("orange", colors.get("purple")))
        
        # Determine workspace active color (usually primary accent)
        active_color = colors.get("blue", colors.get("yellow", colors.get("green")))
        
        css = f"""/* {theme_name} Theme for Waybar */
...
@define-color grey0 {colors['grey0']};
@define-color grey1 {colors['grey1']};
@define-color grey2 {colors['grey2']};

/* Smart Theme Colors */
@define-color hover-color {hover_color};
@define-color active-color {active_color};
"""
