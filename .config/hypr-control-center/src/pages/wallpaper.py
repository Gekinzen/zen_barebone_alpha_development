...
Matching Appearance page design + Current wallpaper highlight
"""
...
class WallpaperPage:
    ...
    def __init__(self, window):
        ...
        self.thumbnail_widgets = {}  # Map wallpaper_path -> thumbnail box
        ...
        # Load saved slideshow state AND current wallpaper from JSON
        wallpaper_data = self.prefs.load()
        self.slideshow_enabled = wallpaper_data.get('slideshow_enabled', False)
        
        # IMPORTANT: Load current wallpaper from JSON using correct key 'current'
        self.current_wallpaper = wallpaper_data.get('current', '')
        
        # If no saved wallpaper, try to get from swww query
        if not self.current_wallpaper:
            try:
                ...
                if result.returncode == 0:
                    # Parse output: "eDP-1: /path/to/wallpaper.jpg"
                    output = result.stdout.strip()
                    if ':' in output:
                        self.current_wallpaper = output.split(':', 1)[1].strip()
            except:
                pass
        
        print(f"\n[INIT] ========================================")
        print(f"[INIT] Loaded from JSON:")
        print(f"[INIT]   current_wallpaper = '{self.current_wallpaper}'")
        print(f"[INIT]   type = {type(self.current_wallpaper)}")
        print(f"[INIT]   length = {len(self.current_wallpaper)}")
        print(f"[INIT] ========================================\n")
    ...
    def _render_page(self):
        ...
        print(f"\n[DEBUG] === RENDERING PAGE {self.current_page + 1} ===")
        print(f"[DEBUG] Current wallpaper to match: '{self.current_wallpaper}'")
        print(f"[DEBUG] Type: {type(self.current_wallpaper)}")
        ...
        # Render thumbnails
        for i, wallpaper_path in enumerate(self.wallpapers[start_idx:end_idx]):
            thumbnail = self._create_thumbnail(wallpaper_path)
            wallpaper_str = str(wallpaper_path)

            # Append thumbnail
            self.flowbox.append(thumbnail)

            # GTK4-safe way to get FlowBoxChild
            flow_child = self.flowbox.get_last_child()

            # Store FlowBoxChild
            self.thumbnail_widgets[wallpaper_str] = flow_child

            is_match = (wallpaper_str == self.current_wallpaper)

            print(f"[DEBUG] [{i}] {wallpaper_path.name}")
            print(f"[DEBUG]      Path: '{wallpaper_str}'")
            print(f"[DEBUG]      Match: {is_match}")

            if is_match and flow_child is not None:
                thumbnail.add_css_class('current-wallpaper')
                self.flowbox.select_child(flow_child)
                print("[DEBUG]      ✅ AUTO-SELECTED CURRENT WALLPAPER")



        print(f"[DEBUG] Total on page: {len(self.thumbnail_widgets)}")
        print(f"[DEBUG] === END RENDER ===\n")

        # Update pagination
        self.page_label.set_text(
            f"Page {self.current_page + 1} of {max(1, self.total_pages)}"
        )
        self.prev_btn.set_sensitive(self.current_page > 0)
        self.next_btn.set_sensitive(self.current_page < self.total_pages - 1)

    def _sync_selection_to_current(self):
        """Force FlowBox selection to match current wallpaper"""

        if not self.current_wallpaper or not self.wallpapers:
            return
        ...
        try:
            index = self.wallpapers.index(Path(self.current_wallpaper))
        except ValueError:
            return

        target_page = index // WALLPAPERS_PER_PAGE
        ...
        # Select + highlight current
        flow_child = self.thumbnail_widgets.get(self.current_wallpaper)
        if flow_child:
            self.flowbox.select_child(flow_child)
            flow_child.get_child().add_css_class('current-wallpaper')

            print("[SYNC] ✅ Selection + highlight forced to current wallpaper")


    def _clear_all_selections(self):
        """Remove all FlowBox selections and highlights"""
        self.flowbox.unselect_all()

        for flow_child in self.thumbnail_widgets.values():
            if flow_child:
                box = flow_child.get_child()
                if box and box.has_css_class('current-wallpaper'):
                    box.remove_css_class('current-wallpaper')
    ...
    def _on_slideshow_toggle(self, value: bool):
        """Handle slideshow toggle - SAVE STATE"""
        self.slideshow_enabled = value
    ...
    def _on_random_toggle(self, value: bool):
        """Handle random transition toggle"""
        self.prefs.update({'random_transition': value})
        self.window._show_toast("Random transitions: " + ("On" if value else "Off"))
    ...
    def _update_current_highlight(self, new_wallpaper_path: str):
        """Update highlight to show current wallpaper + save to JSON"""
        print(f"\n[DEBUG] === UPDATE HIGHLIGHT ===")
        print(f"[DEBUG] New wallpaper: {new_wallpaper_path}")
        print(f"[DEBUG] Old wallpaper: {self.current_wallpaper}")
        
        # Update internal state FIRST
        self.current_wallpaper = new_wallpaper_path
        
        # Remove highlight from all VISIBLE thumbnails
        removed_count = 0
        print(f"[DEBUG] Checking {len(self.thumbnail_widgets)} thumbnails...")
        for path, widget in list(self.thumbnail_widgets.items()):
            if widget.get_parent() is not None:  # Check if still attached
                if widget.has_css_class('current-wallpaper'):
                    widget.remove_css_class('current-wallpaper')
                    removed_count += 1
                    print(f"[DEBUG] ❌ Removed highlight from: {Path(path).name}")
        
        print(f"[DEBUG] Removed {removed_count} old highlights")
        
        # Add highlight to new current (if visible on current page)
        if new_wallpaper_path in self.thumbnail_widgets:
            widget = self.thumbnail_widgets[new_wallpaper_path]
            if widget.get_parent() is not None:  # Check if still attached
                widget.add_css_class('current-wallpaper')
                print(f"[DEBUG] ✅ Added highlight to: {Path(new_wallpaper_path).name}")
            else:
                print(f"[DEBUG] ⚠️ Widget not attached!")
        else:
            print(f"[DEBUG] ⚠️ Current wallpaper not on this page")
            print(f"[DEBUG] Available paths:")
            for path in list(self.thumbnail_widgets.keys())[:3]:
                print(f"[DEBUG]   - {path}")
        
        # CRITICAL: Save to JSON with correct key 'current'
        self.prefs.update({'current': new_wallpaper_path})
        print(f"[DEBUG] Saved to JSON")
    ...
    def _apply_wallpaper(self, wallpaper_path: str, transition: str = 'fade'):
        """Apply wallpaper using swww and update highlight + save to JSON"""
        try:
            subprocess.run([
                'swww', 'img',
                wallpaper_path,
            ...
            # CRITICAL: Save to JSON using 'current' key directly
            # Don't use set_current_wallpaper(), use update() directly!
            self.prefs.update({'current': wallpaper_path})
            
            
            # CRITICAL: Update highlight (removes old, adds new)
            self._update_current_highlight(wallpaper_path)
            ####pra mag update 
            self._sync_selection_to_current()

            
            print(f"[Wallpaper] Applied and saved: {wallpaper_path}")
