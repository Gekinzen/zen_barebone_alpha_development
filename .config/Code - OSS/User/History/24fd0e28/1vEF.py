...
class AppearanceView(Adw.PreferencesPage):
    def __init__(self):
        ...
        active = Gtk.Scale.new_with_range(Gtk.Orientation.HORIZONTAL, 0, 1, 0.01)
        inactive = Gtk.Scale.new_with_range(Gtk.Orientation.HORIZONTAL, 0, 1, 0.01)
        ...
        active.connect("value-changed", lambda *_: apply())
        inactive.connect("value-changed", lambda *_: apply())

        deco.add(Adw.ActionRow(title="Active Opacity", activatable_widget=active))
        deco.add(Adw.ActionRow(title="Inactive Opacity", activatable_widget=inactive))
