    ...
    def _create_thumbnail(self, image_path: Path) -> Gtk.Box:
        ...
        try:
            ...
            if scaled_w > target or scaled_h > target:
                offset_x = max(0, (scaled_w - target) // 2)
                offset_y = max(0, (scaled_h - target) // 2)
                
                # Make sure we don't go out of bounds
                if offset_y + target <= scaled_h and offset_x + target <= scaled_w:
                    ...
                    # If dimensions wrong, just use scaled
