...
class WallpaperPage:
    ...
    def _create_thumbnail(self, image_path: Path) -> Gtk.Box:
        """Create thumbnail that ACTUALLY FILLS 240x240"""
        box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=4)
        box.wallpaper_path = str(image_path)
        
        # Fixed size frame - FORCE 240x240
        frame = Gtk.Frame()
        frame.set_size_request(240, 240)
        frame.add_css_class('wallpaper-frame')
        
        try:
            # Load and scale to FILL 240x240
            pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_scale(
                str(image_path),
                240, 240,  # Target size
                False  # Don't preserve aspect - FILL THE BOX!
            )
            
            # Create image widget - FORCE SIZE
