...
class WallpaperPage:
    ...
    def _create_thumbnail(self, image_path: Path) -> Gtk.Box:
        """Create thumbnail with image that FILLS entire 240x240"""
        box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=8)
        box.wallpaper_path = str(image_path)
        
        # Create overlay to force exact size
        overlay = Gtk.Overlay()
        ...
        try:
            # Load image - FORCE scale to 240x240, don't preserve aspect!
            pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_scale(
                str(image_path),
                240, 240,
                False  # preserve_aspect_ratio = FALSE!
            )
            
            # Create image that takes full space
