    ...
    Rectangle {
        ...
        ColumnLayout {
            ...
            Rectangle {
                ...
                // Drag-to-move. Commits geometry on release.
                MouseArea {
                    id: moveArea
                    anchors.fill: parent
        ...
        Rectangle {
            ...
            MouseArea {
                ...
                onPositionChanged: (m) => {
                    if (!pressed) return
                    const nw = _ow + (m.x - _px)
                    const nh = _oh + (m.y - _py)
                    panel.width  = Math.max(DesktopIconsState.widgetMinW, nw)
                    panel.height = Math.max(DesktopIconsState.widgetMinH, nh)
                }
                onReleased: {
                    DesktopIconsState.setWidgetGeometry(
                        panel.x, panel.y, panel.width, panel.height)
