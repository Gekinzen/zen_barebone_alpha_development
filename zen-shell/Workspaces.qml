    ...
    GridLayout {
        ...
        Repeater {
            ...
            delegate: Rectangle {
                ...
                readonly property int _dotBase: Math.max(6, Math.round(PanelState.workspaceDotInactive * 0.55))

                Layout.preferredWidth: Math.round((_dots
                    ? (isActive ? PanelState.workspaceDotActive : _dotBase)
                    : (isActive ? PanelState.workspaceDotActive : PanelState.workspaceDotInactive)) * wsRoot._fit)
                Layout.preferredHeight: Math.round((_dots
                    ? _dotBase
                    : (isActive ? PanelState.workspaceDotActive : PanelState.workspaceDotInactive)) * wsRoot._fit)
