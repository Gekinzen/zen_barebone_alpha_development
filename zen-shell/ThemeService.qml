...
Singleton {
    ...
    // v8.0.0-alpha-hf160 — raw theme values (written by the theme system). The PUBLIC
    // fg/grey* below are COMPUTED from these so the Glass+ look can auto-contrast text.
    property color _fgBase:    "#c0caf5"
    property color _grey0Base: "#9aa5ce"
    property color _grey1Base: "#737aa2"
    property color _grey2Base: "#565f89"
    // On the Glass+ (clear) look, text auto-contrasts against the effective panel: dark
    // ink on a light/bright wallpaper, white on a dark one. Every component already reads
    // fg / grey0 / grey1 / grey2, so redefining them HERE adapts the whole shell from ONE
    // place — no 114-file sweep. On every other look they equal the theme values exactly,
    // so nothing regresses. LookService.clearInk* read only PanelState + WallpaperServiceV5
    // (never ThemeService), so there is no binding cycle.
    readonly property color fg:    LookService.isClear ? LookService.clearInk      : _fgBase
    readonly property color grey0: LookService.isClear ? LookService.clearInkDim    : _grey0Base
    readonly property color grey1: LookService.isClear ? LookService.clearInkFaint  : _grey1Base
    readonly property color grey2: LookService.isClear ? LookService.clearInkFaint  : _grey2Base
    ...
    function saveAsCustomTheme(customName: string) {
        ...
        const data = {
            ...
            colors: {
                ...
                bg3: colorToHex(bg3),
                bg4: colorToHex(bg4),
                fg:    colorToHex(_fgBase),
                grey0: colorToHex(_grey0Base),
                grey1: colorToHex(_grey1Base),
                grey2: colorToHex(_grey2Base),
                red:    colorToHex(red),
    ...
    function setAccent(name: string, hex: string) {
        ...
        else if (name === "fg")    _fgBase = hex
        else if (name === "grey0") _grey0Base = hex
        else if (name === "grey1") _grey1Base = hex
        else if (name === "grey2") _grey2Base = hex
    ...
    function applyJson(text) {
        ...
        try {
            ...
            if (c.bg3) bg3 = c.bg3
            if (c.bg4) bg4 = c.bg4
            if (c.fg)    _fgBase    = c.fg
            if (c.grey0) _grey0Base = c.grey0
            if (c.grey1) _grey1Base = c.grey1
            if (c.grey2) _grey2Base = c.grey2
            if (c.red)    red    = c.red
    ...
    // v8.0.0-alpha-hf150 — fuzzel glass alpha argument. On the clear Shell Look,
    // map glassStrength to the same 2-digit hex the hyprbars use (a touch of body
    // so the launcher stays readable); otherwise restore an opaque background.
    function _fuzzelGlassArg() {
        try {
            if (typeof LookService !== "undefined" && LookService.isClear) {
                // v8.0.0-alpha-hf178 — this used to hand fuzzel 0.30 - 0.14*strength, so at
                // the hf173 default (level 5) the launcher was white at 16% alpha. The shell's
                // panels get away with that because Hyprland BLURS them; fuzzel's surface has
                // no blur rule, so 16% is not frost — it's a window you can see straight
                // through, with the desktop behind bleeding into the app list. And fuzzel has
                // no text-outline option to rescue it the way Text.Outline rescues our panels.
                // So the launcher gets body, exactly like popupColor() gives menus body: same
                // white-dominant sheet, legible on anything. Frost still varies with the
                // slider, just across a range that can actually be read.
                var st = Math.max(0, Math.min(1, PanelState.glassStrength))
                var a = 0.97 - 0.05 * st        // level 1 -> 0.97, level 5 -> 0.92
    ...
    // re-run the fuzzel alpha when the look or frost changes (theme regen also
    // fires it, but a pure look switch doesn't retheme the terminal).
    Connections {
        target: (typeof LookService !== "undefined") ? LookService : null
        ignoreUnknownSignals: true
        function onIsClearChanged() { fuzzelGlass.running = false; fuzzelGlass.running = true }
        function onGlassFillChanged() { if (LookService.isClear) { fuzzelGlass.running = false; fuzzelGlass.running = true } }
    }
    Process {
        id: fuzzelGlass
        command: ["bash", "-c",
            "if [ -x \"$HOME/.local/bin/zen-fuzzel-glass.sh\" ]; then " +
            "  \"$HOME/.local/bin/zen-fuzzel-glass.sh\" " + root._fuzzelGlassArg() + " > /tmp/zen-fuzzel-glass.log 2>&1 || true; " +
            "fi"]
        running: false
    }
    ...
    Process {
        ...
        command: ["bash", "-c",
            ...
            "fi; " +
            // v8.0.0-alpha-hf150 — regen writes an OPAQUE fuzzel background; on the
            // clear look, follow up and thin its alpha so the launcher is glass too.
            "if [ -x \"$HOME/.local/bin/zen-fuzzel-glass.sh\" ]; then " +
            "  \"$HOME/.local/bin/zen-fuzzel-glass.sh\" " + root._fuzzelGlassArg() + " >> /tmp/zen-theme-regen.log 2>&1 || true; " +
            "fi"]
        running: false
    }
