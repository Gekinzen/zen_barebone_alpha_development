...
import Qt5Compat.GraphicalEffects   // v8.0.0-alpha-hf146: ColorOverlay (same module the avatars use)
...
Rectangle {
    ...
    // v8.0.0-alpha-hf182 — which surface this instance is mounted on. ZenDock sets
    // "dock"; Bar and BarVertical leave it at "bar". Without this the two shared every
    // setting, because they are literally the same component.
    property string surface: "bar"
    ...
    // v6.16.4.12.6: Frosted bg — alpha lets Hyprland layer blur through.
    // v8.0.0-alpha-hf181 — that alpha was hardcoded at 0.32 and unreachable; it's
    // PanelState.taskbarOpacity now, and it can go to 0. At 0 the plate AND its border
    // AND the inner highlight all stop drawing, or you'd be left with an outline round
    // nothing. On the clear look the plate uses the same white-dominant tint every other
    // surface has used since hf173 (this Rectangle is hand-rolled Qt.rgba, which is why
    // that sweep never saw it).
    readonly property real plateAlpha: (taskbarRoot.surface === "dock" || PanelState.taskbarLinkSurfaces)
                                       ? PanelState.taskbarOpacity
                                       : PanelState.taskbarBarOpacity
    color: taskbarRoot.plateAlpha <= 0.001
           ? "transparent"
           : (LookService.isClear
              ? Qt.rgba(LookService.clearSurfaceBase.r, LookService.clearSurfaceBase.g,
                        LookService.clearSurfaceBase.b, taskbarRoot.plateAlpha)
              : Qt.rgba(ThemeService.bg0.r, ThemeService.bg0.g, ThemeService.bg0.b,
                        taskbarRoot.plateAlpha))
    border.width: taskbarRoot.plateAlpha <= 0.001 ? 0 : 1
    border.color: ThemeService.alpha(ThemeService.fg,
                                     0.10 * Math.min(1.0, taskbarRoot.plateAlpha / 0.32))

    // Subtle inner highlight for depth — fades out with the plate.
    Rectangle {
        anchors.fill: parent
        anchors.margins: 1
        radius: parent.radius - 1
        color: "transparent"
        visible: taskbarRoot.plateAlpha > 0.001
        border.width: 1
        border.color: ThemeService.alpha(ThemeService.fg,
                                         0.04 * Math.min(1.0, taskbarRoot.plateAlpha / 0.32))
        z: 0
    }
    ...
    GridLayout {
        ...
        Item {
            ...
            Rectangle {
                ...
                // v8.0.0-alpha-hf178 — pills off = the macOS-style floating dock. Idle
                // icons get no plate at all; hover keeps a faint wash so you can still
                // see what you're pointing at, and "running" is already said by the dot
                // under the icon, which is exactly how macOS says it too.
                color: LookService.taskbarIconBackgrounds
                       ? (isActive
                          ? ThemeService.blue
                          : ma.containsMouse
                            ? ThemeService.bg3
                            : LookService.surfaceColor(ThemeService.bg1, 0.85))
                       : (ma.containsMouse
                          ? LookService.surfaceColor(ThemeService.bg1, 0.55)
                          : "transparent")
                ...
                // v8.0.0-alpha-hf146 — "white lahat ng icons?"
                //
                // Off → the normal colourful themed icon. On → a ColorOverlay that
                // flattens every opaque pixel to iconTint (white on the clear look)
                // while preserving the icon's alpha, so the silhouette survives and
                // only the colour is unified. ColorOverlay is Qt5Compat.Graphical-
                // Effects — the SAME module the profile avatars (OpacityMask) use in
                // five shipped files, so it resolves on any real Quickshell build.
                property bool _mono: (typeof LookService !== "undefined") && LookService.monoIcons

                Image {
                    id: appIconImg
                    anchors.centerIn: parent
                    // hf182 — icon scale applies inside the button box, so it can't
                    // change the bar's layout or push icons into overflow.
                    width: Math.round(taskbarRoot.btnSize * 0.6 * PanelState.taskbarIconScale)
                    height: Math.round(taskbarRoot.btnSize * 0.6 * PanelState.taskbarIconScale)
                    ...
                    visible: !parent._mono            // hidden while the overlay draws
                }

                ColorOverlay {
                    anchors.fill: appIconImg
                    source: appIconImg
                    color: (typeof LookService !== "undefined") ? LookService.iconTint : Qt.rgba(1,1,1,1)
                    visible: parent._mono
                    cached: true
                }
                ...
                Rectangle {
                    ...
                    Text {
                        style: LookService.isClear ? Text.Outline : Text.Normal
                        styleColor: LookService.clearTextOutline
                         anchors.centerIn: parent; text: windowCount; color: Theme.bg0; font.pixelSize: 9; font.bold: true }
                }
                ...
                PopupWindow {
                    ...
                    Rectangle {
                        anchors.fill: parent
                        radius: 12
                        // v8.0.0-alpha-hf150 — right-click context menu follows the look.
                        color: LookService.popupColor(0.96)
                            ...
                            Rectangle {
                                ...
                                RowLayout {
                                    ...
                                    Text {
                                        style: Text.Normal
                                        styleColor: LookService.clearTextOutline
                                         text: isPinned ? taskbarRoot.nfUnpin : taskbarRoot.nfPin; color: LookService.popupInk; font.family: Theme.monoFont; font.pixelSize: 14 }
                                    Text {
                                        style: Text.Normal
                                        styleColor: LookService.clearTextOutline
                                         text: isPinned ? "Unpin" : "Pin to taskbar"; color: LookService.popupInk; font.family: Theme.fontFamily; font.pixelSize: 12 }
                                }
                            ...
                            Rectangle {
                                ...
                                RowLayout {
                                    ...
                                    Text {
                                        style: Text.Normal
                                        styleColor: LookService.clearTextOutline
                                         text: taskbarRoot.nfWindow; color: LookService.popupInk; font.family: Theme.monoFont; font.pixelSize: 14 }
                                    Text {
                                        style: Text.Normal
                                        styleColor: LookService.clearTextOutline
                                         text: "New window"; color: LookService.popupInk; font.family: Theme.fontFamily; font.pixelSize: 12 }
                                }
                            ...
                            Rectangle {
                                ...
                                RowLayout {
                                    ...
                                    Text {
                                        style: Text.Normal
                                        styleColor: LookService.clearTextOutline
                                         text: taskbarRoot.nfClose; color: ThemeService.red; font.family: Theme.monoFont; font.pixelSize: 14 }
                                    Text {
                                        style: Text.Normal
                                        styleColor: LookService.clearTextOutline
                                         text: windowCount > 1 ? "Close all (" + windowCount + ")" : "Close"; color: ThemeService.red; font.family: Theme.fontFamily; font.pixelSize: 12 }
                                }
