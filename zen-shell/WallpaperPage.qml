...
ScrollView {
    ...
    // v8.0.0-alpha-hf164 — auto-load: rescan the folder whenever the page becomes
    // visible, so newly added wallpapers show up without hitting refresh, and any
    // stale search is cleared. The scan is a cheap `find`, safe to re-run on open.
    onVisibleChanged: if (visible && typeof WallpaperServiceV5 !== "undefined") {
        WallpaperServiceV5.searchQuery = ""
    ...
    ColumnLayout {
        ...
        SettingsSection {
            ...
            // v8.0.0-alpha-hf164 — fuzzy search across the whole folder. Filters
            // WallpaperServiceV5.filteredList live (subsequence match) and jumps to
            // page 1 on every keystroke so results are always on screen.
            RowLayout {
                Layout.fillWidth: true
                ...
                Text {
                    ...
                    font.pixelSize: 13
                    color: LookService.textDimColor(ThemeService.grey1)
                    style: LookService.isClear ? Text.Outline : Text.Normal
                    styleColor: LookService.clearTextOutline
                ...
                TextField {
                    id: searchField
                    Layout.fillWidth: true
                    placeholderText: "Search wallpapers…  (fuzzy — try \"frst\" for forest)"
                    font.family: Theme.fontFamily
                    font.pixelSize: 12
                    text: WallpaperServiceV5.searchQuery
                    onTextChanged: {
                        if (WallpaperServiceV5.searchQuery !== text) {
                    ...
                    delegate: Rectangle {
                        ...
                        ColumnLayout {
                            ...
                            Rectangle {
                                ...
                                radius: 6
                                color: LookService.surfaceColor(ThemeService.bg2, 1.0)
                                clip: true
