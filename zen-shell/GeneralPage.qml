...
ScrollView {
    ...
    ColumnLayout {
        ...
        HMSection {
            ...
            // v8.0.0-alpha-hf129: freeze the monitor before the overlay maps.
            HMRow {
                visible: ZenStringsState.enabled && ZenStringsState.screenshotRopeEnabled
                label: "Freeze the screen"
                description: "Snapshot the whole monitor before the overlay opens, then crop that still. "
                           + "Keeps open menus, popups and the Control Center in the shot instead of "
                           + "losing them when the overlay takes focus. Also makes capture instant."
                HMSwitch {
                    checked: ZenStringsState.screenshotFreeze
                    onToggled: { ZenStringsState.screenshotFreeze = checked; ZenStringsState.markDirty() }
                }
            }

            // v8.0.0-alpha-hf130: where the four ropes hang from.
            HMRow {
                visible: ZenStringsState.enabled && ZenStringsState.screenshotRopeEnabled
                label: "Rope origin"
                description: "Band = the ropes pin to the corners of the third of the screen your cursor is in, "
                           + "and glide across as you move. Corners = the classic rig, welded to the four screen "
                           + "corners (a rope can span the whole ultrawide)."
                ZenDropdown {
                    width: 140
                    model: ["band", "corners"]
                    currentIndex: ZenStringsState.ropeOriginMode === "corners" ? 1 : 0
                    onActivated: {
                        ZenStringsState.ropeOriginMode = model[currentIndex]
                        ZenStringsState.markDirty()
                    }
                }
            }
