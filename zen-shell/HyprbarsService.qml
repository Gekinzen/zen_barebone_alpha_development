...
Singleton {
    ...
    // v8.0.0-alpha-hf149 — the title bar tracks the Shell Look. When the look or
    // the frost slider changes, rewrite the hyprbars config so the bar re-colours
    // (translucent white on Glass+, theme bg1 otherwise). Debounced by writeConfig.
    Connections {
        target: (typeof LookService !== "undefined") ? LookService : null
        ignoreUnknownSignals: true
        function onIsClearChanged() { root.writeConfig() }
        function onGlassFillChanged() { if (LookService.isClear) root.writeConfig() }
    }
    ...
    // v8.0.0-alpha-hf149 — glassStrength (0..1) → 2-digit hex alpha for the bar.
    // strength 0 → 0.30 (0x4d), 1 → 0.16 (0x29). Clamped, always two digits.
    function _glassAlphaHex() {
        var st = 0.5
        try { st = Math.max(0, Math.min(1, PanelState.glassStrength)) } catch (e) {}
        var a = 0.30 - 0.14 * st
    ...
    function buildConfigText() {
        // v8.0.0-alpha-hf149 — on Glass+ the title bar goes translucent white to
        // match the glassy shell (hyprbars supports rgba()). The frost strength
        // rides the same glassStrength slider, mapped to a slightly stronger alpha
        // than the panels since a thin title bar needs a touch more body to read.
        const barColor = (typeof LookService !== "undefined" && LookService.isClear)
            ? "rgba(ffffff" + root._glassAlphaHex() + ")"
            : root._colorOrDefault("bg1", "rgb(282828)")
