...
Singleton {
    ...
    // v8.0.0-alpha-hf144 — play a soft chime when a notification toast appears.
    // Default on; one toggle in Settings → General. Throttled like the rest.
    ...
    // v8.0.0-alpha-hf145 — per-event throttle. A volume tick is a discrete click
    // that should feel continuous under a fast drag; 80ms let a 60fps drag through
    // at only ~20% cadence, so adjustments felt silent. 45ms is the floor before
    // canberra samples start to overlap. Login/logout keep the wider window — they
    // are one-shots and never drag. Falls back to throttleMs for unlisted events.
    readonly property var throttleFor: ({
        "volume-change": 45,
        "mute": 60
    })
    ...
    FileView {
        ...
        onLoaded: {
            try {
                ...
                if (typeof j.playNotificationSound === "boolean") root.playNotificationSound = j.playNotificationSound   // hf144
    ...
    function _save() {
        ...
        const obj = {
            ...
            playNotificationSound: root.playNotificationSound,
            volume: root.volume
        }
    ...
    onPlayNotificationSoundChanged: saveDebounce.restart()   // hf144
    ...
    function play(event) {
        ...
        const win = root.throttleFor[event] !== undefined ? root.throttleFor[event] : root.throttleMs
