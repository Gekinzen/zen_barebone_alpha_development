...
Rectangle {
    ...
    // v8.0.0-alpha-hf147 — clear look: frosted white body, no border.
    ...
    // v8.0.0-alpha-hf193 — the vertical bar had the identical fault, rotated
    // ninety degrees: two fillHeight spacers between the three zones, so the
    // CENTRE zone drifted by (top - bottom) / 2 whenever either end changed.
    // Fixed the same way and for the same reason — see the long note in
    // Bar.qml. Doing only the horizontal bar would have left the bug alive
    // for anyone who moves their bar to the side.
    Item {
        id: rootColV
        anchors.fill: parent
        anchors.topMargin: 8
        ...
        // ── v8.0.0-alpha-hf193 — implicitHeight must be computed by hand now ──
        //
        // This was a ColumnLayout, which derives implicitHeight from its
        // children for free. A plain Item does not — it reports 0 — and
        // `vFitScale` above is bound to exactly that value:
        //
        //     rootColV.implicitHeight > rootColV.height  ?  height/implicit : 1.0
        //
        // With implicitHeight stuck at 0 the condition can never be true, the
        // scale silently pins to 1.0, and a full vertical bar goes back to
        // overflowing off the screen instead of shrinking to fit. The auto-fit
        // would have been lost without a single error anywhere.
        //
        // Same sum the ColumnLayout was making: three zones plus the two gaps
        // between them.
        implicitHeight: topColV.implicitHeight
                        + centerColV.implicitHeight
                        + bottomColV.implicitHeight
                        + zoneGap * 2

        implicitWidth: Math.max(topColV.implicitWidth,
                                centerColV.implicitWidth,
                                bottomColV.implicitWidth)

        readonly property int zoneGap: 10
        readonly property real centerMaxHeight: Math.max(0,
            2 * Math.min(height / 2 - topColV.height    - zoneGap,
                         height / 2 - bottomColV.height - zoneGap))
        ...
        // CENTRE zone — holds the true middle, narrows rather than slides.
        Item {
            id: centerSlotV
            anchors.verticalCenter: parent.verticalCenter
            anchors.horizontalCenter: parent.horizontalCenter
            width: centerColV.implicitWidth
            height: Math.min(centerColV.implicitHeight, rootColV.centerMaxHeight)
            clip: height < centerColV.implicitHeight
            visible: height > 0
