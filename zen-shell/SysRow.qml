...
Item {
    ...
    function fmtModule(icon, glyph, textVal) {
        // v8.0.0-alpha-hf184 — Paul: "paki lagyan ng spacing icon between sa text".
        // \u2002 (en space) is a touch wider than a normal space, so the glyph and its
        // value don't crowd. Applied to both the value and the bargraph forms.
    ...
    GridLayout {
                        ...
                        Item {
                            ...
                            Rectangle {
                                ...
                                color: spkMuteMa.containsMouse
                                       ? ThemeService.bg3
                                       : LookService.surfaceColor(ThemeService.bg2, 0.7)
                            ...
                            Column {
                                ...
                                Item {
                                    ...
                                    Rectangle {
                                        ...
                                        antialiasing: true   // v7.0.0-beta.1-hf99: smooth circle knob (Qt won't AA rounded corners by default)
        ...
        SysRowIcon {
            ...
            // v8.0.0-alpha-hf177: the glyph above was correct all along — it
            // was `wifiConnected` underneath it that never went true, so a
            // joined wifi box drew 󰤮/󰈀. Fixed in ConnectivityService; the
            // tooltip now also reports an in-flight connect.
            tipTitle: ConnectivityService.wifiBusy
                      ? (ConnectivityService.wifiBusyVerb + "\u2026")
                      : (ConnectivityService.wifiConnected
                         ? ("WiFi Connected — " + ConnectivityService.wifiSSID)
                         : (ConnectivityService.lanConnected ? "Ethernet Connected" : "Network Disconnected"))
            tipDetail: {
                if (ConnectivityService.wifiConnected)
            ...
            // ── v8.0.0-alpha-hf195 ──
            // Left opens the GTK Wi-Fi selector, which is what this icon is
            // for nine times in ten. The Control Center it used to open is
            // still one right-click away, and is also on SUPER+C — nothing
            // was taken away, the more useful action just moved to the front.
