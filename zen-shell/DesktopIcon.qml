...
Item {
    ...
    Rectangle {
        ...
        readonly property string _iconSource: {
            ...
            // Stage -1 (v8.0.0-alpha-hf134): the user's custom PNG wins.
            //
            // DesktopIconsWidget has honoured `customIcons` since hf85; this
            // file never did. So right-clicking a tile to set a custom icon
            // worked in Single-widget mode and silently did nothing in
            // free-form mode. Same state, same precedence, both renderers.
            const custom = DesktopIconsState.customIcons
                ? (DesktopIconsState.customIcons[root.entry.name] || "") : ""
