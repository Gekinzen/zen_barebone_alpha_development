        ...
        SettingsSection {
            ...
            RowLayout {
                ...
                Rectangle {
                    ...
                    radius: 12
                    color: LookService.surfaceColor(ThemeService.bg1, 1.0)
                    border.width: 2
                ...
                contentItem: ColumnLayout {
                    ...
                    Item {
                        ...
                        Canvas {
                            ...
                            onPaint: {
                                ...
                                if (w <= 0 || h <= 0) return
                                // v7.0.0-beta.1-hf99: paint at the CURRENT lightness with
                                // exact per-sat HSL stops → plane matches the emitted color.
                                ...
                                for (let xx = 0; xx < w; xx += 2) {
                                    ...
                                    for (let i = 0; i <= STOPS; i++) {
                                        const t = i / STOPS
                                        grad.addColorStop(t, Qt.hsla(hueVal, 1.0 - t, L, 1.0))
                                    }
                                    ctx.fillStyle = grad
                            ...
                            MouseArea {
                                id: svMouse2
                                anchors.fill: parent
                                // v7.0.0-beta.1-hf99: preventStealing + explicit
                                // svMouse2.pressed so drag works, not just click.
                                preventStealing: true
                                acceptedButtons: Qt.LeftButton
                                function pick(mx, my) {
                                    hsCanvas.pickerHue = Math.max(0, Math.min(1, mx / hsCanvas.width))
                                    hsCanvas.pickerSat = Math.max(0, Math.min(1, 1.0 - my / hsCanvas.height))
                                    pickerHexInput.text = hexEditor._currentHex()
                                }
                                onPressed: function(mouse) { pick(mouse.x, mouse.y) }
                                onPositionChanged: function(mouse) {
                                    if (svMouse2.pressed) pick(mouse.x, mouse.y)
                                }
                            }
                    ...
                    RowLayout {
                        ...
                        ZenSlider {
                            ...
                            // v7.0.0-beta.1-hf99: keep the plane WYSIWYG as L changes
                            // (covers both drag and programmatic set from hex entry).
                            onValueChanged: {
