...
Singleton {
    ...
    // v8.0.0-alpha-hf134 — panel chrome. `true` is the light frosted card from
    // Paul's mockup; `false` is the dark slab hf83 shipped. Purely cosmetic,
    // and reversible, so nothing is taken away.
    property bool widgetLightGlass: true

    // v8.0.0-alpha-hf144 — let the Shell Look drive the desktop panel. On by
    // default so Glass/Glass+ reach it; turn off to keep widgetLightGlass manual.
    property bool lookApplyDesktop: true
    //
    ...
    function _loadFromJson(raw) {
        ...
        try {
            ...
            if (typeof s.widgetLightGlass === "boolean") widgetLightGlass = s.widgetLightGlass   // hf134
            if (typeof s.lookApplyDesktop === "boolean") lookApplyDesktop = s.lookApplyDesktop   // hf144
    ...
    onWidgetLightGlassChanged: markDirty()      // hf134
    onLookApplyDesktopChanged: markDirty()      // hf144
    ...
    // ── v8.0.0-alpha-hf134 — the picker moved here ──
    //
    // It used to live inside DesktopIconsWidget, which meant only the
    // Single-widget panel could open it. The settings page needs it too, and
    // so does free-form mode. A singleton is the right home for a thing that
    // mutates singleton state.
    //
    // zenity → kdialog → xdg-desktop-portal via `zenity`'s absence. Whichever
    // the user has. Nothing here assumes a desktop environment.
    property string _pickTarget: ""
    signal iconPickFailed(string reason)
    ...
    Process {
        ...
        command: ["bash", "-c",
            ...
            "  zenity --file-selection --title='Pick an icon image' " +
            "    --file-filter='Images | *.png *.svg *.jpg *.jpeg *.webp *.ico' 2>/dev/null; " +
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                if (out === "__NO_PICKER__") {
                    console.warn("[DesktopIconsState] no file picker found (install zenity or kdialog)")
                    root.iconPickFailed("Install zenity or kdialog to pick an icon file.")
                } else if (out.length > 0 && root._pickTarget.length > 0) {
                    root.setCustomIcon(root._pickTarget, out)
                }
                root._pickTarget = ""
            }
        }
