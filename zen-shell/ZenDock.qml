...
Rectangle {
    ...
    // v8.0.0-alpha-hf112: preset-written, slider-owned (no override).
    radius: DockState.syncFromBar
        ? (Theme.styleMode === "round" ? 22 : Theme.barRadius)
        : DockState.overrideCornerRadius

    // v8.0.0-alpha-hf147 — on the clear look the body is one frosted white fill
    // (LookService.bodyColor), no theme tint, no border. Off the clear look this
    // whole expression is the caller's own value, unchanged. See LookService.
    ...
    border.color: LookService.bodyBorderColor(
        DockState.syncFromBar ? PanelState.borderColor : DockState.overrideBorderColor)
    border.width: LookService.bodyBorderWidth(
        !DockState.borderEnabled
            ? 0
            : (DockState.syncFromBar
    ...
    Component { id: cTaskbar;       Taskbar       { surface: "dock" } }   // hf182
    ...
    Component { id: cClock;         Clock         { inDock: true } }
    Component { id: cBattery;       Battery       {} }
    Component { id: cNotifIcon;     NotificationIcon {} }
    // v8.0.0-alpha-hf126 — ZenWeather is the same component the bar's "weather"
    // module uses. One renderer, two surfaces.
    Component { id: cWeather;       ZenWeather    {} }
    Component { id: cPower;         DockPowerButton {} }
    ...
    function getComponent(name) {
        switch (name) {
            ...
            case "weather":       return cWeather       // hf126
            case "power":         return cPower         // hf126
    ...
    Rectangle {
        ...
        radius: 6
        color: LookService.surfaceColor(ThemeService.bg0, 0.6)
        Text {
