...
ScrollView {
    ...
    ColumnLayout {
        ...
        ColumnLayout {
            ...
            Text {
                ...
                styleColor: LookService.clearTextOutline
                 text: "Bar Modules"; font.family: Theme.fontFamily; font.pixelSize: 22; font.weight: Font.Bold; color: ThemeService.fg }
            Text {
                style: LookService.isClear ? Text.Outline : Text.Normal
                styleColor: LookService.clearTextOutline
                 text: "Format options for clock, workspaces, and font family"; font.family: Theme.fontFamily; font.pixelSize: 12; color: ThemeService.grey1 }
        }
        ...
        // ═══ CLOCK ═══
        // ── v8.0.0-alpha-hf131 — weather icon style for the bar + dock ──
        HMSection {
            title: "Weather"

            HMRow {
                label: "Icon style"
                description: "Which glyph set the bar and dock weather modules draw. "
                           + "Material matches the Glance blob; Emoji matches the dashboard "
                           + "and Quick Settings cards; Nerd uses the Weather Icons set."
                icon: "\ue312"          // wi-cloudy
                separator: true
                ZenDropdown {
                    id: wxIconCombo
                    width: root.dropdownWidth
                    model: ["Material", "Emoji", "Nerd Font"]
                    currentIndex: PanelState.weatherIconStyle === "emoji" ? 1
                                : PanelState.weatherIconStyle === "nerd"  ? 2 : 0
                    onActivated: {
                        PanelState.weatherIconStyle = ["material", "emoji", "nerd"][currentIndex]
            ...
            HMRow {
                label: "Icon colour"
                description: "Condition = a fixed weather palette (sun gold, rain blue, storm violet), "
                           + "which flips to a darker set on a light theme. Accent = one colour for "
                           + "everything, the pre-hf132 look. Emoji icons always carry their own colour."
                icon: "\ue30d"          // wi-day-sunny
                separator: true
                enabled: PanelState.weatherIconStyle !== "emoji"
                ZenDropdown {
                    width: root.dropdownWidth
                    model: ["Condition", "Accent"]
                    currentIndex: PanelState.weatherIconTint === "accent" ? 1 : 0
                    onActivated: {
                        PanelState.weatherIconTint = ["condition", "accent"][currentIndex]
                        PanelState.saveState()
                    }
                }
            }
            ...
            HMRow {
                label: "Windy above"
                description: "WMO has no \"windy\" code, so it is derived: a quiet sky "
                           + "(clear, cloudy or fog) plus wind at or over this speed reads "
                           + "as windy. Rain, snow and storms always win."
                icon: "\ue31e"          // wi-windy
                NumericStepper {
                    from: 5; to: 80; stepSize: 5; suffix: " km/h"
                    value: WeatherService.windyThresholdKmh
                    onValueEdited: v => {
                        WeatherService.windyThresholdKmh = Math.round(v)
            ...
            HMRow {
                ...
                ZenDropdown {
                    ...
                    property var presetIds: ["numbers","korean","chinese","japanese","roman",
                                             "nerd-dots","nerd-circles","classic-dots","nerd-squares","symbols","empty","custom"]
