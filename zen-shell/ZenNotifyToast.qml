    ...
    ColumnLayout {
        ...
        Repeater {
            ...
            delegate: Rectangle {
                ...
                // v8.0.0-alpha-hf144: the toast follows the Shell Look now.
                // It was hardcoded to bg1 @ 0.95, so Glass/Glass+ left the panels
                // clear but every notification stayed a solid box. radius tracks
                // the look; the clear look drops the tint to a white frost.
                radius: (typeof LookService !== "undefined" && PanelState.lookApplyNotifications)
                        ? Math.round(12 * LookService.radiusScale) : 12

                color: (typeof LookService !== "undefined" && PanelState.lookApplyNotifications)
                       ? LookService.panelColor(ThemeService.bg1, PanelState.notificationOpacity)
                ...
                border.color: {
                    ...
                    // hf144: on the clear look, a neutral crisp border keeps the
                    // toast legible without reintroducing colour.
                    if (typeof LookService !== "undefined" && LookService.isClear)
                        return ThemeService.alpha(ThemeService.fg, 0.32)
                ...
                Component.onCompleted: {
                    ...
                    // v8.0.0-alpha-hf146 — the arrival chime is gone. hf144 added
                    // it; Paul asked for it out ("alisin mo tingg"). Notifications
                    // are silent again. The volume tick (hf145) is unaffected.
                }
                Behavior on opacity {
