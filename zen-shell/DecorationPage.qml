    ...
    ColumnLayout {
        ...
        HMSection {
            ...
            HMRow {
                ...
                ColorSwatch {
                    // v8: shadowColor is CSS #RRGGBBAA (alpha last). Show only the
                    // RRGGBB in the swatch, and re-attach the existing alpha on edit.
                    value: "#" + SettingsStateV2.shadowColor.replace(/^#/, "").substring(0, 6)
                    onValueEdited: hex => {
                        var a = SettingsStateV2.shadowColor.replace(/^#/, "").substring(6, 8) || "ee"
                        var c = "#" + hex.replace(/^#/, "").substring(0, 6) + a
                        SettingsStateV2.shadowColor = c
                        SettingsStateV2.hyprctlColor("decoration:shadow:color", c)
                    }
                }
            }
            ...
            HMRow {
                ...
                ColorSwatch {
                    value: "#" + SettingsStateV2.shadowColorInactive.replace(/^#/, "").substring(0, 6)
                    onValueEdited: hex => {
                        var a = SettingsStateV2.shadowColorInactive.replace(/^#/, "").substring(6, 8) || "ee"
                        var c = "#" + hex.replace(/^#/, "").substring(0, 6) + a
                        SettingsStateV2.shadowColorInactive = c
                        SettingsStateV2.hyprctlColor("decoration:shadow:color_inactive", c)
                    }
                }
            }
        }
