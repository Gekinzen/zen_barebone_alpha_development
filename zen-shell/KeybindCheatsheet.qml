...
*
 * ── v8.0.0-alpha-hf192 — FOLLOWS THE SHELL LOOK ──
 *
 * Every colour here used to be a Qt.rgba() literal: a fixed dark card with
 * fixed white text. It ignored the active theme completely, ignored Glass,
 * and on a light theme it was a black slab with white-on-white edges.
 *
 * All of them now route through ThemeService/LookService — the card uses the
 * same `surfaceColor()` as every other panel, which is what actually applies
 * the Glass treatment, and text uses textColor/textDimColor/textFaintColor so
 * the clear-look outlines apply here too.
 */
Rectangle {
    id: cheatsheet
    anchors.fill: parent
    color: ThemeService.alpha(ThemeService.bg0, 0.62)
    visible: false
        ...
        ColumnLayout {
            ...
            RowLayout {
                ...
                Text {
                    ...
                    font.family: "Adwaita Sans"
                    font.pixelSize: 10
                    color: LookService.textFaintColor(ThemeService.grey2)
                }
            }
