...
Singleton {
    ...
    //   "controlcenter" — opens the Zen Control Center (hf126; was a
    //                     notify-send stub from hf82k until then)
    //   "weather"       — ZenWeather, the bar's weather module (hf126)
    //   "power"         — power menu popup: lock/suspend/logout/
    //                     restart/shutdown (hf126)
    //   "tray"          — system tray
    //   "clock"         — clock display
    ...
    // v8: master dock-border toggle. When false the dock has NO
    // border regardless of sync/independent mode (lets you disable
    // the dock border even while the bar keeps its own).
    ...
    /**
     * ══ v8.0.0-alpha-hf192 — HOW MUCH SCREEN EDGE THE DOCK OWNS ══
     *
     * ZenDock's implicitHeight is `height * min(iconSizeScale, 1.6)`, and the
     * plate then sits `marginEdge` in from the screen. Anything that wants to
     * float ABOVE the dock — the volume OSD, toasts, anything transient — has
     * to add those together, and until now each of them either guessed or,
     * more often, ignored the dock entirely and drew straight over it.
     *
     * One number, computed once, so "always above the dock" is a rule rather
     * than a per-widget guess. Zero when the dock is off or on the far edge,
     * so callers can add it unconditionally.
     */
    readonly property int edgeFootprint: enabled
        ? Math.round(height * Math.min(iconSizeScale > 0 ? iconSizeScale : 1.0, 1.6))
          + marginEdge
        : 0

    readonly property int bottomFootprint: (enabled && isBottom) ? edgeFootprint : 0
    readonly property int topFootprint:    (enabled && isTop)    ? edgeFootprint : 0

    // ── v8.0.0-alpha-hf131 — popup anchoring for dock modules ──
    //
    // Same shape as PanelState.popupAnchorEdges / popupAnchorGravity, but keyed
    // off the DOCK's edge instead of the BAR's. DockPowerButton was reading the
    // bar's helpers, so on the default layout — top bar, bottom dock — its menu
    // was told to open downward, off the bottom of the screen, and the
    // compositor flipped it somewhere else entirely.
    //
    // Values match Quickshell's Edges flags as used across this tree:
    //   Top = 1 · Bottom = 2 · Left = 4 · Right = 8
    // A bottom dock grows UP (Edges.Top); a top dock grows DOWN (Edges.Bottom).
    // The dock has no left/right position, so there are only two cases.
    ...
    function _loadFromJson(raw) {
        ...
        try {
            ...
            if (typeof s.overrideBgColor === "string")   overrideBgColor = "#" + s.overrideBgColor.replace(/^#/, "").substring(0, 6)  // v8 colorfix
            if (typeof s.overrideBorderColor === "string") overrideBorderColor = "#" + s.overrideBorderColor.replace(/^#/, "").substring(0, 6)  // v8 colorfix
