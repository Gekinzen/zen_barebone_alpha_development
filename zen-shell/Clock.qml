...
Rectangle {
    ...
    // v8: true when this clock instance lives in the dock
    // (set by ZenDock). Drives calendar anchoring.
    property bool inDock: false
    ...
    // v8.0.0-alpha-hf150 — the clock module follows the look. On Glass+ its idle
    // body is the frosted white (was bg0 @ 0.9, which read black over a light
    // wallpaper); hover still flips to the blue accent.
    color: ma.containsMouse || PanelState.calendarVisible
           ? ThemeService.alpha(ThemeService.blue, 0.25)
           : LookService.bodyColor(LookService.surfaceColor(ThemeService.bg0, 0.9))
    ...
    function reportPositionToPanelState() {
        ...
        // StartMenu logic).
        let barScreenX = 0
        // v8: the dock is a centered island — derive its screen
        // offset from the dock window width, not the bar panel mode.
        if (root.inDock) {
            barScreenX = (screenW - (win.width || screenW)) / 2
        } else if (PanelState.panelMode === "island") {
    ...
    MouseArea {
        ...
        onClicked: (mouse) => {
            ...
            // v8: tell the calendar which surface (bar/dock) opened it
            PanelState.calendarFromDock = root.inDock
